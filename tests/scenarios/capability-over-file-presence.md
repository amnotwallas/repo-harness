...
The audit recognizes the existing equivalent context, produces no file-presence finding, and grounds any other finding in a concrete capability failure.
...
The skill checked the existing sources of truth, reported the context capability as available, and made no recommendation based only on the absent paths.
